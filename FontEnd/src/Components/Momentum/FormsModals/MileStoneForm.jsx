...
import { createMilestoneReq } from '../../../Redux/Actions/milestone.action';
...
function MileStoneForm({ isOpen, onClose, outcome }) {
  ...
  const onSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      await dispatch(createMilestoneReq(outcomeData));
            ...
            {({ isSubmitting }) => (
              <Form className="p-3">
                <div className="form-group mb-3">
                  <label htmlFor="title" className="form-label">MileStone</label>
                  <Field type="text" name="title" className="form-control" placeholder="Enter MileStone title" />
